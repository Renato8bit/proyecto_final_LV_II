...
class SelectorApp:
    def __init__(self, window, main_window, callback_iniciar_juego=None):
        self.window = window
        ...
        if callback_iniciar_juego is None:
            print("ADVERTENCIA: No se proporcionó callback_iniciar_juego")
    ...
    def crear_interfaz(self):
        ...
        info_label = ttk.Label(self.window, 
                              text="Selecciona 3 Pros y 3 Contras para balancear tu personaje",
                              font=('Helvetica', 10))
    ...
    def iniciar_juego(self):
        ...
        # Verificar si se seleccionaron exactamente 3 pros y 3 contras
        if len(self.selected_pros) != max_pros or len(self.selected_contras) != max_contras:
            respuesta = messagebox.askyesno(
                "Confirmar", 
                f"Debes seleccionar exactamente {max_pros} Pros y {max_contras} Contras.\n"
                f"Actualmente tienes: {len(self.selected_pros)} Pros y {len(self.selected_contras)} Contras.\n"
        ...
        # Usar callback si existe, de lo contrario usar el método antiguo
        if self.callback_iniciar_juego:
            self.callback_iniciar_juego(nombre, self.selected_pros, self.selected_contras)
        else:
